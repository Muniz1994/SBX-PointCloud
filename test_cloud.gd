...
func _ready() -> void:
	...
	cloud.file_path = "res://Examples/Merged_clouds.laz"
...
func _on_loaded(point_count: int) -> void:
	...
	print("Format ID  : ", cloud.get_reader().get_point_format_id())
	print("Load time  : %.1f ms" % cloud.get_reader().get_load_time_ms())
	print("Chunks     : ", cloud.get_chunk_count())
	print("Build time : %.1f ms" % cloud.get_build_time_ms())
